...
Document* pretty_project(Project proj, Allocator* a) {
    ...
    PtrArray nodes = mk_ptr_array(8, a);
    {
        ...
        push_ptr(mk_str_doc(view_name_string(proj.package.name), a), &name_nodes);
...

    {
        ...
        NameArray deps = proj.package.dependencies;
        for (size_t i = 0; i < deps.len; i ++) {
            push_ptr(mk_str_doc(view_name_string(deps.data[i]), a), &sub_nodes);
        }
