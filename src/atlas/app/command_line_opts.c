...
static AtlasCommand internal_parse_command(StringArray args, ArenaAllocator* a) {
    ...
    if (string_cmp(subcommand, mv_string("init")) == 0) {
        ...
        if (args.len > 2) {
            return (AtlasCommand) {
                .type = CInvalid,
                .error_message = mv_string("'atlas run' must take at most 1 argument, the name of the target to run."),
            };
        } else {
            return (AtlasCommand) {
              .type = CRun,
              .run.target = {
                  .type = args.len == 2 ? Some : None,
                  .val = args.len == 2 ? args.data[1] : (String){},
              }
            };
        }
        ...
        if (args.len != 2) {
            return (AtlasCommand) {
                .type = CInvalid,
                .error_message = mv_string("'atlas build' must take at most 1 argument, the name of the target to build."),
            };
        } else {
            return (AtlasCommand) {
                .type = CBuild,
