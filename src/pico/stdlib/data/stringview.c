...
void add_stringview_module(Target target, Module *data, RegionAllocator* region) {
    ...
    ModuleHeader header = (ModuleHeader) {
        .name = string_to_symbol(mv_string("stringview")),
        .imports = imports,
        .exports = exports,
    };
    ...
    const char* str_type = "(def StringView Named String Struct [.memsize U64] [.bytes Address])";
    compile_toplevel(str_type, module, target, &point, &pi_point, region);

    const char *str_nth_byte =
        "(def nth-byte proc [(idx U64) (string StringView)] \n"
    ...
    /* const char *str_pushutf8 = */
    /*     "(def push-utf8 proc [(idx U64) (string (Ptr StringView))] \n" */
    /*     "  ()\n" */
    /*     "  (load {U8} (num-to-address (u64.+ idx (address-to-num string.bytes)))))"; */
    /* compile_toplevel(str_pushutf8, module, target, &point, &pi_point, region); */

    Result r = add_module_def(data, string_to_symbol(mv_string("stringview")), module);
