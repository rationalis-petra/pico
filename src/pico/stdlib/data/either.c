...
void add_either_module(Target target, Module *data, PiAllocator* module_allocator, RegionAllocator* region) {
    ...
    compile_toplevel(mk_either_type, module, target, &point, &pi_point, region);

    const char *mk_left_fn =
        "(def left all [A B] proc [(x A)] (Either A B):left x)";
    compile_toplevel(mk_left_fn, module, target, &point, &pi_point, region);

    const char *mk_right_fn =
        "(def right all [A B] proc [(y B)] (Either A B):right y)";
    compile_toplevel(mk_right_fn, module, target, &point, &pi_point, region);
