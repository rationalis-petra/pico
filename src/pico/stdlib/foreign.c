...
void build_dynlib_open_fn(PiType* type, Assembler* ass, PiAllocator* pia, Allocator* a, ErrorPoint* point) {
    ...
    CType dynlib_result = mk_struct_ctype(
        ...
        mk_union_ctype(pia, 2,
                       "error_message", mk_string_ctype(pia),
                       "dynlib", mk_voidptr_ctype(pia)));

    // Proc type
    CType fn_ctype = mk_fn_ctype(pia, 1, "path", mk_string_ctype(pia), dynlib_result);
...
void build_dynlib_close_fn(PiType* type, Assembler* ass, PiAllocator* pia, Allocator* a, ErrorPoint* point) {
    // Proc type
    CType fn_ctype = mk_fn_ctype(pia, 1, "lib", copy_c_type(mk_voidptr_ctype(pia), pia), c_void);
...
void build_dynlib_symbol_fn(PiType* type, Assembler* ass, PiAllocator* pia, Allocator* a, ErrorPoint* point) {
    // here, we use void pointers because we don't need the  
    // C API to check everything for us.
    // DynLibResult
    ...
    CType fn_ctype = mk_fn_ctype(pia, 2,
                                 "lib", mk_voidptr_ctype(pia),
                                 "symbol", mk_string_ctype(pia),
                                 symbol_result);
