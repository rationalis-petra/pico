...
void generate_size_of(Regname dest, PiType* type, AddressEnv* env, Assembler* ass, Allocator* a, ErrorPoint* point) {
    ...
    if (sz_res == Ok) {
        ...
        case TTile: {
            generate_align_of(R9, type->tile.element, env, ass, a, point);
            build_unary_op(Push, reg(R9, sz_64), ass, a, point);
            generate_size_of(R8, type->tile.element, env, ass, a, point);
            ...
            build_binary_op(Mov, reg(RCX, sz_64), imm64(total_arr_len(type->tile.dimensions)), ass, a, point);
...
void generate_align_of(Regname dest, PiType* type, AddressEnv* env, Assembler* ass, Allocator* a, ErrorPoint* point) {
    ...
    if (al_res == Ok) {
        ...
        case TTile: {
            generate_align_of(dest, type->tile.element, env, ass, a, point);
            break;
        }
        case TStruct: {
            build_unary_op(Push, imm32(0), ass, a, point);
