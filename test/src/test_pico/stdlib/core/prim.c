...
void run_pico_stdlib_core_prim_tests(TestLog *log, Module* module, Environment* env, Target target, RegionAllocator* region) {
    ...
    if (test_start(log, mv_string("and-ff"))) {
        uint8_t expected = 0;
        TEST_EQ("(prim.bool.and :false :true)");
    }
    ...
    if (test_start(log, mv_string("and-tt"))) {
        uint8_t expected = 1;
        TEST_EQ("(prim.bool.and :true :true)");
    }

    if (test_start(log, mv_string("or-ff"))) {
        uint8_t expected = 0;
        TEST_EQ("(prim.bool.or :false :false)");
    }

    if (test_start(log, mv_string("or-ft"))) {
        uint8_t expected = 1;
        TEST_EQ("(prim.bool.or :false :true)");
    }

    if (test_start(log, mv_string("or-tt"))) {
        uint8_t expected = 1;
        TEST_EQ("(prim.bool.or :true :true)");
    }

    if (test_start(log, mv_string("not-t"))) {
        bool expected = false;
        TEST_EQ("(prim.bool.not :true)");
    }

    if (test_start(log, mv_string("not-f"))) {
        bool expected = true;
        TEST_EQ("(prim.bool.not :false)");
    }
}
