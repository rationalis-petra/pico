...
void run_assembler_tests(TestLog *log, Allocator *a) {
    ...
    if (test_start(log, mv_string("asm-add-r64-imm8"))) { // Add RAX, 12
        ASM_TEST() {
            ...
            build_binary_op(Add, reg(RAX, sz_64), imm8(12), ass, &arena, &point);
    ...
    if (test_start(log, mv_string("asm-add-r64-r64"))) { // Add RBX, RCX
        ASM_TEST() {
            ...
            build_binary_op(Add, reg(RBX, sz_64), reg(RCX, sz_64), ass, &arena, &point);
    ...
    if (test_start(log, mv_string("asm-add-r64-m64"))) { // Add RDX, [R9 + 8]
        ASM_TEST() {
            ...
            build_binary_op(Add, reg(RDX, sz_64), rref8(R9, 8, sz_64), ass, &arena, &point);
    ...
    if (test_start(log, mv_string("asm-add-r8-m8"))) { // Add DL, [R9 + 8]b
        ASM_TEST() {
            ...
            build_binary_op(Add, reg(RDX, sz_8), rref8(R9, 8, sz_8), ass, &arena, &point);
    ...
    if (test_start(log, mv_string("asm-mov-m8-r8"))) { // Mov [RSI + 8]b, DIL, 
        ASM_TEST() {
            ...
            build_binary_op(Mov, rref8(RSI, 8, sz_8), reg(RDI, sz_8), ass, &arena, &point);
    ...
    if (test_start(log, mv_string("movss-xmm32-xmm32"))) { // MovSS XMM0, XMM1
        ASM_TEST() {
            ...
            build_binary_op(MovSS, reg(XMM0, sz_32), reg(XMM1, sz_32), ass, &arena, &point);
    ...
    if (test_start(log, mv_string("movss-xmm32-m32"))) { // MovSS XMM0, XMM1
        ASM_TEST() {
            ...
            build_binary_op(MovSS, reg(XMM2, sz_32), rref8(RSP, 8, sz_32), ass, &arena, &point);
    ...
    if (test_start(log, mv_string("movss-m32-xmm32"))) { // MovSS XMM0, XMM1
        ASM_TEST() {
            ...
            build_binary_op(MovSS, rref8(RDI, 8, sz_32), reg(XMM5, sz_32), ass, &arena, &point);
    ...
    if (test_start(log, mv_string("movsd-xmm64-m64"))) { // MovSD XMM0, XMM1
        ASM_TEST() {
            ...
            build_binary_op(MovSD, reg(XMM0, sz_64), rref8(RSP, 8, sz_64), ass, &arena, &point);
